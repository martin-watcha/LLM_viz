...
import {
  ...
  Layers, FastForward, Rewind, ArrowLeft,
} from 'lucide-react';
...
const TransformerView = ({ onBack }) => {
  const [tIndex, setTIndex] = useState(0);
  const [subStepIndex, setSubStepIndex] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [speed, setSpeed] = useState(1500);

  const currentPhase = STEPS_DATA[tIndex];
  const currentS     = currentPhase.substeps[subStepIndex];
  ...
  }, [tIndex, subStepIndex, currentPhase.substeps.length]);
  ...
  const totalSteps = STEPS_DATA.reduce((acc, p) => acc + p.substeps.length, 0);
    ...
    <div className="flex flex-col h-full">
      ...
      <header className="flex items-center justify-between px-4 py-3 bg-white border-b border-slate-200 select-none">
        <div className="flex items-center gap-3">
          <button onClick={onBack} className="p-1.5 rounded-full hover:bg-slate-100 text-slate-500 transition-colors">
            <ArrowLeft size={18} />
          </button>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col bg-slate-50 overflow-y-auto min-w-0">
          <FlowDiagram currentPhaseIdx={tIndex} />
          <FormulaBar currentS={currentS} />
      ...
      <footer className="bg-white border-t border-slate-200 relative shrink-0 select-none">
        ...
        <div className="p-3 flex items-center justify-between max-w-4xl mx-auto gap-4">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => jumpToStep(0, 0)} className="p-1.5 rounded hover:bg-slate-100 text-slate-400"><RotateCcw size={14} /></button>
            <button onClick={handlePrev} disabled={tIndex === 0 && subStepIndex === 0} className="p-1.5 rounded hover:bg-slate-100 text-slate-600 disabled:opacity-30"><ChevronLeft size={18} /></button>
            <button
            ...
            <button onClick={handleNext} disabled={tIndex === STEPS_DATA.length - 1 && subStepIndex === currentPhase.substeps.length - 1} className="p-1.5 rounded hover:bg-slate-100 text-slate-600 disabled:opacity-30"><ChevronRight size={18} /></button>
          </div>
